...
![SimReady](https://img.shields.io/badge/Output-SimReady%20USD-76b900.svg)
...
- ✅ **SimReady 호환 USD** — USDPhysics(Collider · Mass · Material) + 시맨틱 라벨 + assetInfo 자동 주입 (Omniverse Kit 불필요)
...
6. 익스포트 포맷: USD (Omniverse 권장)
7. ☑ SimReady 자산으로 출력 — USDPhysics 자동 주입
8. ▶ Generate → 약 70초
9. Blender 열기 → 🎨 Blender로 보내기
10. Omniverse / Twinmotion / BIM 협업툴에서 import → 시뮬레이션 즉시 가능
```

## SimReady 출력이란?

OpenUSD 기반의 NVIDIA SimReady 스펙 호환 자산입니다. 일반 USD는 정적
지오메트리만 담지만 SimReady는 시뮬레이션 가능한 메타까지 포함합니다.

| 항목 | 일반 USD | **SimReady USD (이 프로젝트)** |
|---|---|---|
| 메시 + 정점 색상 | ✅ | ✅ |
| Stage upAxis=Y, metersPerUnit=1.0 | 종종 누락 | ✅ 강제 |
| 기본 prim · Kind="component" | 없음 | ✅ |
| USDPhysics Collider (convex hull) | 없음 | ✅ |
| USDPhysics MassAPI + 카테고리별 density | 없음 | ✅ (steel/wood/aluminum 자동) |
| USDPhysics MaterialAPI (마찰·반발) | 없음 | ✅ |
| 시맨틱 라벨 (semanticLabel) | 없음 | ✅ |
| assetInfo (name · version · 제조사 · 시리얼) | 없음 | ✅ |
| 실치수 mm → 미터 스케일 보정 | 없음 | ✅ (dimensions_mm 입력시) |

**Omniverse Kit 불필요** — `pxr` (Blender 5.x 번들의 USD Python) 만 사용.
공식 NVIDIA aif-pipeline validate는 Kit 설치된 환경에서 옵션 stage로
실행됩니다 (Kit 없으면 자동 스킵, 내장 pxr-only 셀프체크로 대체).
